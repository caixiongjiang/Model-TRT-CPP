...
async def segmentation_predict(input_data: ImageInput = Body(...),
    ...
    try:
        ...
        logger.info("Request ID: {}, Info message, file_path: {}".format(request_id, file_path))
        # 图片前处理
        image_tensor = segmentation_model.preprocess_image(imagePath=file_path)
        logger.info("Request ID: {}, Info message: The pre-processing of the picture is completed.".format(request_id))
        # 图片推理
        prediction = segmentation_model.inference(image_tensor)
        logger.info("Request ID: {}, Info message: Image inference was completed.".format(request_id))
        # 后处理
        image = segmentation_model.postprocess(results=prediction, classNameFile="./data/classname/orange_defect.names", legend=False, save=False)
        logger.info("Request ID: {}, Info message: Image postprocess was completed.".format(request_id))
        # 图像转化为base64
        img_byte_array = io.BytesIO()
        image.save(img_byte_array, format='PNG')
...
if __name__ == "__main__":
    ...
    segmentation_model = segmentModel(modelName=model_params["SEGMENT_MODEL_NAME"])
    # 先使用模型推理一遍
    ...
    segmentation_model.inference(image_tensor=torch.randn(1, 3, 224, 224).cuda())
